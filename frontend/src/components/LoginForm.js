...
import { TextField, Typography, Button, InputAdornment, makeStyles, CssBaseline, Grid, Link as Mlink } from '@material-ui/core'
...
export default function LoginForm(props) {
    ...
    const onSubmit = (e) => {
        if (Validator.validate(email) === false) { setEmailFailed(true) }
        else { setEmailFailed(false) }

        if (password === '') { setPasswordFailed(true) }
        else { setPasswordFailed(false) }

        console.log(emailFailed, passwordFailed);

        if (emailFailed === false && passwordFailed === false)
    ...
    return (
        <main className={classes.content}>
            ...
            <Grid container direction='column' justify='center' alignItems='center'>
                ...
                <TextField
                    error={emailFailed}
                    helperText={emailFailed ? "Invalid email address" : ""}
                    name="email"
                    label="Email"
                    variant="outlined"
                    margin="normal"
                ...
                <TextField
                    ...
                    helperText={passwordFailed ? "Password may not by empty" : ""}
                    name="password"
                    label="Password"
                    variant="outlined"
                    margin="normal"
                ...
                />
                <Button style={{ marginTop: 16, padding: 5, fontSize: 22, borderRadius: "50px", width: "260px" }} type='submit' color="primary" variant="contained" onClick={onSubmit}>
                    Log in
                    </Button>
                <Typography style={{ padding: 5, fontSize: 16 }}>
                    I forgot <Mlink className={classes.link} component={Link} to="/">my password</Mlink>
                </Typography>
                <Typography style={{ padding: 30, fontSize: 20 }}>
                    Don't have a Lorelines account? <Mlink className={classes.link} component={Link} to="/">Click here</Mlink>
                </Typography>
            </Grid>
        </main>
    );
